	...
	<div>
		<v-container fluid grid-list-md style="background-color: #757575">
			<v-layout row wrap justify-content-center my-3 mx-3>
				...
				<v-layout class="row ml-4 mt-4">
					...
					<v-layout class="d-block" style="margin-left: 60px">
						<v-card width="324">
							<v-card-text>
								<v-layout row wrap>
									<v-flex xs12 md12>
										<label class="v-btn primary v-btn--block" style="padding: 6px; cursor: pointer">
								            <input
								            type="file"
								    ...
								    <!-- <v-flex xs12 md12>
								    	<v-btn @click="sheet = !sheet">click</v-btn>
								    </v-flex> -->
								    <v-flex xs12 md12>
							...
							<v-card-text>
								<v-layout row wrap>
									...
									<v-flex xs12 sm12 md12>
										<v-btn color="primary" @click="posterDialog = true" :disabled="published">{{ $t('labels.publish')}}</v-btn>
											...
											<v-card>
												<v-container>
													...
													<v-card-actions>
														...
														<v-btn color="primary" @click="renameImage(element)">{{ $t('buttons.save') }}</v-btn>
													</v-card-actions>
												</v-container>
											</v-card>
										</v-dialog>
		...
		<v-bottom-sheet v-model="sheet">
		    <v-tabs>
		      ...
		      <v-tab-item>
		        <v-card height="400" style="overflow-y: scroll">
		          <v-card-text>
		            <v-layout row wrap>
						<v-flex xs12 sm12 md12>
							<label class="v-btn primary" style="padding: 6px; margin-bottom: 10px; cursor: pointer">
	...
	import draggable from 'vuedraggable'
  	...
  	export default {
	    ...
	    mounted(){
	    	...
	    	axios.get(settings.root_url + '/api/poster-editing/' + this.$route.params.id)
            .then(response => {
            	...
            	if(response.data.poster.poster_id == -1)
            		self.published = false;
            	else
            		self.published = true;

            	self.fonts = response.data.fonts;
            	var fonts = self.fonts.map(function(el){ return el.name });
            	WebFont.load({
				    google: {
				      	families: fonts
				    }
	    ...
	    methods: {
		    ...
		    onTypeChange(){
	            ...
	            .then(response => {
	                ...
	                console.log(this.posterForm.availableGroups);
		    		return;
	            });
		    },
